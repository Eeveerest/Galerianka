...
require_once 'database.php';
  ...
  $filter = $_POST['filter'];
   ...
   if ($filter!='none'){
    switch ($filter) {
      case "filter_one":
        $stmt = $pdo->prepare('SELECT id, name, price FROM `products` inner JOIN products_categories on products.id = products_categories.product_id WHERE products_categories.category_id = 2
    ...
    $stmt = $pdo->prepare('SELECT id, name, price, picture FROM `products` inner JOIN products_categories on products.id = products_categories.product_id WHERE products_categories.category_id = 2
      and name like :name;');
  }
