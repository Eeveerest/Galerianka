...
$stmt = $pdo->query('SELECT p.id, name, price FROM `products`p INNER JOIN order_products op on op.product_id = p.id group by p.id order by count(op.product_id) desc limit 4');
